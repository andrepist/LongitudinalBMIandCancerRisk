...
pop.analysis <- pop.analysis %>% select(-c("bmi_18", "bmi_30" ,                             "bmi_40"                            ,  "bmi_55"                    ,         
                                           ...
                                           "bmi_38",                              "bmi_39" , 
                                           cancer_NA, n))
# adjust duration for age of onset
...
for (cancer in canceres){
  ...
  for (exposure in exposures){
    ...
    if(cancer %in%c("cancer_Breast post","cancer_Breast pre" )){
      if(exposure == "yearsBMI25more"){
        test <- current.pop %>%  
          do(model = coxph(Surv(age.index, age.index + follow_up, get(cancer)) ~
                             get(exposure) + qmedea + nationality + smoking +
                             alcohol+ageOnsetOver +strata(age_gr),  data = .))  
      }else{
        if(exposure == "yearsBMI30more"){
          test <- current.pop %>%  
            do(model = coxph(Surv(age.index, age.index + follow_up, get(cancer)) ~
                               get(exposure) + qmedea + nationality + smoking +
                               alcohol+ageOnsetObese+ strata(age_gr),  data = .))  
        }else{
...
saveRDS(df, "out.ajustageduration.rds")

out<-readRDS("out.95.rds")
df<-readRDS("out.ajustageduration.rds")
...
toplotSD <- out %>% 
  filter(!exposure%in%c("ageOnsetOver", "ageOnsetObese", "yearsBMI30more", "yearsBMI25more")) %>% 
   rbind(df) %>% 
...
toplotSD <-toplotSD %>% separate(cancer, into=c(NA, "cancer"), sep="_") %>% 
  mutate(HR=ifelse(expo=="Duration overweight", exp(estimate*sd((pop.analysis%>%filter(everOver==1))$yearsBMI25more)),
                   ifelse(expo=="Duration obesity", exp(estimate*sd((pop.analysis%>%filter(everObese==1))$yearsBMI30more)),
                                               ...
                                               ifelse(expo=="Age onset obesity", exp(estimate*sd(pop.analysis$ageOnsetObese, na.rm = T)), 
                                                      ifelse(expo=="Yearly BMI change", exp(estimate*sd(pop.analysis$trajectorySlope)),
                                                             ifelse(expo=="BMI at index", exp(estimate*sd(pop.analysis$bmi_atindex)),
                                                                    "Missing")))))))),
         low=ifelse(expo=="Duration overweight", exp(conf.low*sd((pop.analysis%>%filter(everOver==1))$yearsBMI25more)),
                    ifelse(expo=="Duration obesity", exp(conf.low*sd((pop.analysis%>%filter(everObese==1))$yearsBMI30more)),
         ...
         up=ifelse(expo=="Duration overweight", exp(conf.high*sd((pop.analysis%>%filter(everOver==1))$yearsBMI25more)),
                   ifelse(expo=="Duration obesity", exp(conf.high*sd((pop.analysis%>%filter(everObese==1))$yearsBMI30more)),
...
women <-toplotSD %>% filter(cancer%in%c( "Cervix Uteri","Corpus Uteri","Ovary")) %>% 
  mutate(HR=ifelse(expo=="Duration overweight", exp(estimate*sd((pop.analysis%>%filter(sexe=="D", everOver==1))$yearsBMI25more)),
                   ifelse(expo=="Duration obesity", exp(estimate*sd((pop.analysis%>%filter(sexe=="D", everObese==1))$yearsBMI30more)),
                                               ...
                                               ifelse(expo=="Age onset obesity", exp(estimate*sd((pop.analysis%>%filter(sexe=="D"))$ageOnsetObese, na.rm = T)), 
                                                      ifelse(expo=="Yearly BMI change", exp(estimate*sd((pop.analysis%>%filter(sexe=="D"))$trajectorySlope)),
                                                             ifelse(expo=="BMI at index", exp(estimate*sd((pop.analysis%>%filter(sexe=="D"))$bmi_atindex)),
                                                                    "Missing")))))))),
         low=ifelse(expo=="Duration overweight", exp(conf.low*sd((pop.analysis%>%filter(sexe=="D", everOver==1))$yearsBMI25more)),
                    ifelse(expo=="Duration obesity", exp(conf.low*sd((pop.analysis%>%filter(sexe=="D", everObese==1))$yearsBMI30more)),
         ...
         up=ifelse(expo=="Duration overweight", exp(conf.high*sd((pop.analysis%>%filter(sexe=="D", everOver==1))$yearsBMI25more)),
                   ifelse(expo=="Duration obesity", exp(conf.high*sd((pop.analysis%>%filter(sexe=="D", everObese==1))$yearsBMI30more)),
...
men <- toplotSD %>% filter(cancer%in%c("Prostate", "Testis")) %>% 
  mutate(HR=ifelse(expo=="Duration overweight", exp(estimate*sd((pop.analysis%>%filter(sexe=="H", everOver==1))$yearsBMI25more)),
                   ifelse(expo=="Duration obesity", exp(estimate*sd((pop.analysis%>%filter(sexe=="H", everObese==1))$yearsBMI30more)),
                                               ...
                                               ifelse(expo=="Age onset obesity", exp(estimate*sd((pop.analysis%>%filter(sexe=="H"))$ageOnsetObese, na.rm = T)), 
                                                      ifelse(expo=="Yearly BMI change", exp(estimate*sd((pop.analysis%>%filter(sexe=="H"))$trajectorySlope)),
                                                             ifelse(expo=="BMI at index", exp(estimate*sd((pop.analysis%>%filter(sexe=="H"))$bmi_atindex)),
                                                                    "Missing")))))))),
         low=ifelse(expo=="Duration overweight", exp(conf.low*sd((pop.analysis%>%filter(sexe=="H", everOver==1))$yearsBMI25more)),
                    ifelse(expo=="Duration obesity", exp(conf.low*sd((pop.analysis%>%filter(sexe=="H", everObese==1))$yearsBMI30more)),
         ...
         up=ifelse(expo=="Duration overweight", exp(conf.high*sd((pop.analysis%>%filter(sexe=="H", everOver==1))$yearsBMI25more)),
                   ifelse(expo=="Duration obesity", exp(conf.high*sd((pop.analysis%>%filter(sexe=="H", everObese==1))$yearsBMI30more)),
...
breast_pre <-toplotSD %>% filter(cancer%in%c( "Breast pre")) %>% 
  mutate(HR=ifelse(expo=="Duration overweight", exp(estimate*sd((pop.analysis%>%filter(sexe=="D",meno_status_index==0, everOver==1))$yearsBMI25more)),
                   ifelse(expo=="Duration obesity", exp(estimate*sd((pop.analysis%>%filter(sexe=="D",meno_status_index==0, everObese==1))$yearsBMI30more)),
                                               ...
                                               ifelse(expo=="Age onset obesity", exp(estimate*sd((pop.analysis%>%filter(sexe=="D",meno_status_index==0))$ageOnsetObese, na.rm = T)), 
                                                      ifelse(expo=="Yearly BMI change", exp(estimate*sd((pop.analysis%>%filter(sexe=="D",meno_status_index==0))$trajectorySlope)),
                                                             ifelse(expo=="BMI at index", exp(estimate*sd((pop.analysis%>%filter(sexe=="D",meno_status_index==0))$bmi_atindex)),
                                                                    "Missing")))))))),
         low=ifelse(expo=="Duration overweight", exp(conf.low*sd((pop.analysis%>%filter(sexe=="D",meno_status_index==0, everOver==1))$yearsBMI25more)),
                    ifelse(expo=="Duration obesity", exp(conf.low*sd((pop.analysis%>%filter(sexe=="D",meno_status_index==0, everObese==1))$yearsBMI30more)),
         ...
         up=ifelse(expo=="Duration overweight", exp(conf.high*sd((pop.analysis%>%filter(sexe=="D",meno_status_index==0, everOver==1))$yearsBMI25more)),
                   ifelse(expo=="Duration obesity", exp(conf.high*sd((pop.analysis%>%filter(sexe=="D",meno_status_index==0, everObese==1))$yearsBMI30more)),
...
breast_post <-toplotSD %>% filter(cancer%in%c( "Breast post")) %>% 
  mutate(HR=ifelse(expo=="Duration overweight", exp(estimate*sd((pop.analysis%>%filter(sexe=="D",meno_status_index==1, everOver==1))$yearsBMI25more)),
                   ifelse(expo=="Duration obesity", exp(estimate*sd((pop.analysis%>%filter(sexe=="D",meno_status_index==1, everObese==1))$yearsBMI30more)),
                                               ...
                                               ifelse(expo=="Age onset obesity", exp(estimate*sd((pop.analysis%>%filter(sexe=="D",meno_status_index==1))$ageOnsetObese, na.rm = T)), 
                                                      ifelse(expo=="Yearly BMI change", exp(estimate*sd((pop.analysis%>%filter(sexe=="D",meno_status_index==1))$trajectorySlope)),
                                                             ifelse(expo=="BMI at index", exp(estimate*sd((pop.analysis%>%filter(sexe=="D",meno_status_index==1))$bmi_atindex)),
                                                                    "Missing")))))))),
         low=ifelse(expo=="Duration overweight", exp(conf.low*sd((pop.analysis%>%filter(sexe=="D",meno_status_index==1, everOver==1))$yearsBMI25more)),
                    ifelse(expo=="Duration obesity", exp(conf.low*sd((pop.analysis%>%filter(sexe=="D",meno_status_index==1, everObese==1))$yearsBMI30more)),
         ...
         up=ifelse(expo=="Duration overweight", exp(conf.high*sd((pop.analysis%>%filter(sexe=="D",meno_status_index==1, everOver==1))$yearsBMI25more)),
                   ifelse(expo=="Duration obesity", exp(conf.high*sd((pop.analysis%>%filter(sexe=="D",meno_status_index==1, everObese==1))$yearsBMI30more)),
...
htmlTable::htmlTable(table %>% select( cancer, expo, HR1,low, up, N))
