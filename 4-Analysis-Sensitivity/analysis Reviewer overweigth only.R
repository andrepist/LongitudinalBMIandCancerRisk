...
pop.analysis <- readRDS("E:/Andrea/Trajectories OBECAN/populationCoxReviewer.rds")
...
# linear ----
exposures <- c("yearsBMILimit",
               "ageOnsetOverLimitLimitLimitLimit",
               "ageOnsetOverLimitLimitLimitLimitLimit" 
)
...
saveRDS(df, "out.ReviewerOverweigthOnly.rds")

df <- readRDS("out.ReviewerOverweigthOnly.rds")

# table ----
Ncasos <- pop.analysis %>% filter(everObese==0,everover==1) %>% 
...
toplotSD <- df %>% 
  #filter(!exposure%in%c("ageOnsetOverLimit", "ageOnsetObese")) %>% 
  # rbind(out1) %>% 
  mutate(expo=recode(exposure,
                     "yearsBMILimit"="Duration overweight",
                   
                     "cumOverLimit"="Cumulative overweight",
                    
                     "ageOnsetOverLimit"="Age onset overweight" 
...
toplotSD <-toplotSD %>% separate(cancer, into=c(NA, "cancer"), sep="_") %>% 
  mutate(HR=ifelse(expo=="Duration overweight", exp(estimate*sd(pop.analysis$yearsBMILimit)),
                   ifelse(expo=="Duration obesity", exp(estimate*sd(pop.analysis$yearsBMI30more)),
                          ifelse(expo=="Cumulative overweight", exp(estimate*sd(pop.analysis$cumOverLimit)), 
                                 ifelse(expo=="Cumulative obesity", exp(estimate*sd(pop.analysis$cumObese)), 
                                        ifelse(expo=="Age onset overweight", exp(estimate*sd(pop.analysis$ageOnsetOverLimit, na.rm = T)), 
                                               ifelse(expo=="Age onset obesity", exp(estimate*sd(pop.analysis$ageOnsetObese, na.rm = T)), 
                                                      ifelse(expo=="Yearly BMI change", exp(estimate*sd(pop.analysis$trajectorySlope)),
                                                             ifelse(expo=="BMI at index", exp(estimate*sd(pop.analysis$bmi_atindex)),
                                                                    "Missing")))))))),
         low=ifelse(expo=="Duration overweight", exp(conf.low*sd(pop.analysis$yearsBMILimit)),
                    ifelse(expo=="Duration obesity", exp(conf.low*sd(pop.analysis$yearsBMI30more)),
                           ifelse(expo=="Cumulative overweight", exp(conf.low*sd(pop.analysis$cumOverLimit)), 
                                  ifelse(expo=="Cumulative obesity", exp(conf.low*sd(pop.analysis$cumObese)), 
                                         ifelse(expo=="Age onset overweight", exp(conf.low*sd(pop.analysis$ageOnsetOverLimit, na.rm = T)), 
         ...
         up=ifelse(expo=="Duration overweight", exp(conf.high*sd(pop.analysis$yearsBMILimit)),
                   ifelse(expo=="Duration obesity", exp(conf.high*sd(pop.analysis$yearsBMI30more)),
                          ifelse(expo=="Cumulative overweight", exp(conf.high*sd(pop.analysis$cumOverLimit)), 
                                 ifelse(expo=="Cumulative obesity", exp(conf.high*sd(pop.analysis$cumObese)), 
                                        ifelse(expo=="Age onset overweight", exp(conf.high*sd(pop.analysis$ageOnsetOverLimit, na.rm = T)), 
                                               ifelse(expo=="Age onset obesity", exp(conf.high*sd(pop.analysis$ageOnsetObese, na.rm = T)), 
                                                      ifelse(expo=="Yearly BMI change", exp(conf.high*sd(pop.analysis$trajectorySlope)),
                                                             ifelse(expo=="BMI at index", exp(conf.high*sd(pop.analysis$bmi_atindex)),
                                                                    "Missing")))))))))%>%  
  mutate(expo2=ifelse(exposure%in%c("yearsBMILimit", "yearsBMI30more"), "Duration",
                      ifelse(exposure%in%c( "cumOverLimit","cumObese" ), "Cumulative",
                             ifelse(exposure%in%c( "ageOnsetOverLimit","ageOnsetObese" ), "Age of onset",
                                    "Slope"))),
         Type=ifelse(exposure%in%c("yearsBMILimit", "cumOverLimit","ageOnsetOverLimit"), "Overweight/Obese",
...
women <-toplotSD %>% filter(cancer%in%c( "Cervix Uteri","Corpus Uteri","Ovary")) %>% 
  mutate(HR=ifelse(expo=="Duration overweight", exp(estimate*sd((pop.analysis%>%filter(sexe=="D"))$yearsBMILimit)),
                   ifelse(expo=="Duration obesity", exp(estimate*sd((pop.analysis%>%filter(sexe=="D"))$yearsBMI30more)),
                          ifelse(expo=="Cumulative overweight", exp(estimate*sd((pop.analysis%>%filter(sexe=="D"))$cumOverLimit)), 
                                 ifelse(expo=="Cumulative obesity", exp(estimate*sd((pop.analysis%>%filter(sexe=="D"))$cumObese)), 
                                        ifelse(expo=="Age onset overweight", exp(estimate*sd((pop.analysis%>%filter(sexe=="D"))$ageOnsetOverLimit, na.rm = T)), 
                                               ifelse(expo=="Age onset obesity", exp(estimate*sd((pop.analysis%>%filter(sexe=="D"))$ageOnsetObese, na.rm = T)), 
                                                      ifelse(expo=="Yearly BMI change", exp(estimate*sd((pop.analysis%>%filter(sexe=="D"))$trajectorySlope)),
                                                             ifelse(expo=="BMI at index", exp(estimate*sd((pop.analysis%>%filter(sexe=="D"))$bmi_atindex)),
                                                                    "Missing")))))))),
         low=ifelse(expo=="Duration overweight", exp(conf.low*sd((pop.analysis%>%filter(sexe=="D"))$yearsBMILimit)),
                    ifelse(expo=="Duration obesity", exp(conf.low*sd((pop.analysis%>%filter(sexe=="D"))$yearsBMI30more)),
                           ifelse(expo=="Cumulative overweight", exp(conf.low*sd((pop.analysis%>%filter(sexe=="D"))$cumOverLimit)), 
                                  ifelse(expo=="Cumulative obesity", exp(conf.low*sd((pop.analysis%>%filter(sexe=="D"))$cumObese)), 
                                         ifelse(expo=="Age onset overweight", exp(conf.low*sd((pop.analysis%>%filter(sexe=="D"))$ageOnsetOverLimit, na.rm = T)), 
         ...
         up=ifelse(expo=="Duration overweight", exp(conf.high*sd((pop.analysis%>%filter(sexe=="D"))$yearsBMILimit)),
                   ifelse(expo=="Duration obesity", exp(conf.high*sd((pop.analysis%>%filter(sexe=="D"))$yearsBMI30more)),
                          ifelse(expo=="Cumulative overweight", exp(conf.high*sd((pop.analysis%>%filter(sexe=="D"))$cumOverLimit)), 
                                 ifelse(expo=="Cumulative obesity", exp(conf.high*sd((pop.analysis%>%filter(sexe=="D"))$cumObese)), 
                                        ifelse(expo=="Age onset overweight", exp(conf.high*sd((pop.analysis%>%filter(sexe=="D"))$ageOnsetOverLimit, na.rm = T)), 
...
men <- toplotSD %>% filter(cancer%in%c("Prostate", "Testis")) %>% 
  mutate(HR=ifelse(expo=="Duration overweight", exp(estimate*sd((pop.analysis%>%filter(sexe=="H"))$yearsBMILimit)),
                   ifelse(expo=="Duration obesity", exp(estimate*sd((pop.analysis%>%filter(sexe=="H"))$yearsBMI30more)),
                          ifelse(expo=="Cumulative overweight", exp(estimate*sd((pop.analysis%>%filter(sexe=="H"))$cumOverLimit)), 
                                 ifelse(expo=="Cumulative obesity", exp(estimate*sd((pop.analysis%>%filter(sexe=="H"))$cumObese)), 
                                        ifelse(expo=="Age onset overweight", exp(estimate*sd((pop.analysis%>%filter(sexe=="H"))$ageOnsetOverLimit, na.rm = T)), 
                                               ifelse(expo=="Age onset obesity", exp(estimate*sd((pop.analysis%>%filter(sexe=="H"))$ageOnsetObese, na.rm = T)), 
                                                      ifelse(expo=="Yearly BMI change", exp(estimate*sd((pop.analysis%>%filter(sexe=="H"))$trajectorySlope)),
                                                             ifelse(expo=="BMI at index", exp(estimate*sd((pop.analysis%>%filter(sexe=="H"))$bmi_atindex)),
                                                                    "Missing")))))))),
         low=ifelse(expo=="Duration overweight", exp(conf.low*sd((pop.analysis%>%filter(sexe=="H"))$yearsBMILimit)),
                    ifelse(expo=="Duration obesity", exp(conf.low*sd((pop.analysis%>%filter(sexe=="H"))$yearsBMI30more)),
                           ifelse(expo=="Cumulative overweight", exp(conf.low*sd((pop.analysis%>%filter(sexe=="H"))$cumOverLimit)), 
                                  ifelse(expo=="Cumulative obesity", exp(conf.low*sd((pop.analysis%>%filter(sexe=="H"))$cumObese)), 
                                         ifelse(expo=="Age onset overweight", exp(conf.low*sd((pop.analysis%>%filter(sexe=="H"))$ageOnsetOverLimit, na.rm = T)), 
         ...
         up=ifelse(expo=="Duration overweight", exp(conf.high*sd((pop.analysis%>%filter(sexe=="H"))$yearsBMILimit)),
                   ifelse(expo=="Duration obesity", exp(conf.high*sd((pop.analysis%>%filter(sexe=="H"))$yearsBMI30more)),
                          ifelse(expo=="Cumulative overweight", exp(conf.high*sd((pop.analysis%>%filter(sexe=="H"))$cumOverLimit)), 
                                 ifelse(expo=="Cumulative obesity", exp(conf.high*sd((pop.analysis%>%filter(sexe=="H"))$cumObese)), 
                                        ifelse(expo=="Age onset overweight", exp(conf.high*sd((pop.analysis%>%filter(sexe=="H"))$ageOnsetOverLimit, na.rm = T)), 
...
breast_pre <-toplotSD %>% filter(cancer%in%c( "Breast pre")) %>% 
  mutate(HR=ifelse(expo=="Duration overweight", exp(estimate*sd((pop.analysis%>%filter(sexe=="D",meno_status_index==0))$yearsBMILimit)),
                   ifelse(expo=="Duration obesity", exp(estimate*sd((pop.analysis%>%filter(sexe=="D",meno_status_index==0))$yearsBMI30more)),
                          ifelse(expo=="Cumulative overweight", exp(estimate*sd((pop.analysis%>%filter(sexe=="D",meno_status_index==0))$cumOverLimit)), 
                                 ifelse(expo=="Cumulative obesity", exp(estimate*sd((pop.analysis%>%filter(sexe=="D",meno_status_index==0))$cumObese)), 
                                        ifelse(expo=="Age onset overweight", exp(estimate*sd((pop.analysis%>%filter(sexe=="D",meno_status_index==0))$ageOnsetOverLimit, na.rm = T)), 
                                               ifelse(expo=="Age onset obesity", exp(estimate*sd((pop.analysis%>%filter(sexe=="D",meno_status_index==0))$ageOnsetObese, na.rm = T)), 
                                                      ifelse(expo=="Yearly BMI change", exp(estimate*sd((pop.analysis%>%filter(sexe=="D",meno_status_index==0))$trajectorySlope)),
                                                             ifelse(expo=="BMI at index", exp(estimate*sd((pop.analysis%>%filter(sexe=="D",meno_status_index==0))$bmi_atindex)),
                                                                    "Missing")))))))),
         low=ifelse(expo=="Duration overweight", exp(conf.low*sd((pop.analysis%>%filter(sexe=="D",meno_status_index==0))$yearsBMILimit)),
                    ifelse(expo=="Duration obesity", exp(conf.low*sd((pop.analysis%>%filter(sexe=="D",meno_status_index==0))$yearsBMI30more)),
                           ifelse(expo=="Cumulative overweight", exp(conf.low*sd((pop.analysis%>%filter(sexe=="D",meno_status_index==0))$cumOverLimit)), 
                                  ifelse(expo=="Cumulative obesity", exp(conf.low*sd((pop.analysis%>%filter(sexe=="D",meno_status_index==0))$cumObese)), 
                                         ifelse(expo=="Age onset overweight", exp(conf.low*sd((pop.analysis%>%filter(sexe=="D",meno_status_index==0))$ageOnsetOverLimit, na.rm = T)), 
         ...
         up=ifelse(expo=="Duration overweight", exp(conf.high*sd((pop.analysis%>%filter(sexe=="D",meno_status_index==0))$yearsBMILimit)),
                   ifelse(expo=="Duration obesity", exp(conf.high*sd((pop.analysis%>%filter(sexe=="D",meno_status_index==0))$yearsBMI30more)),
                          ifelse(expo=="Cumulative overweight", exp(conf.high*sd((pop.analysis%>%filter(sexe=="D",meno_status_index==0))$cumOverLimit)), 
                                 ifelse(expo=="Cumulative obesity", exp(conf.high*sd((pop.analysis%>%filter(sexe=="D",meno_status_index==0))$cumObese)), 
                                        ifelse(expo=="Age onset overweight", exp(conf.high*sd((pop.analysis%>%filter(sexe=="D",meno_status_index==0))$ageOnsetOverLimit, na.rm = T)), 
...
breast_post <-toplotSD %>% filter(cancer%in%c( "Breast post")) %>% 
  mutate(HR=ifelse(expo=="Duration overweight", exp(estimate*sd((pop.analysis%>%filter(sexe=="D",meno_status_index==1))$yearsBMILimit)),
                   ifelse(expo=="Duration obesity", exp(estimate*sd((pop.analysis%>%filter(sexe=="D",meno_status_index==1))$yearsBMI30more)),
                          ifelse(expo=="Cumulative overweight", exp(estimate*sd((pop.analysis%>%filter(sexe=="D",meno_status_index==1))$cumOverLimit)), 
                                 ifelse(expo=="Cumulative obesity", exp(estimate*sd((pop.analysis%>%filter(sexe=="D",meno_status_index==1))$cumObese)), 
                                        ifelse(expo=="Age onset overweight", exp(estimate*sd((pop.analysis%>%filter(sexe=="D",meno_status_index==1))$ageOnsetOverLimit, na.rm = T)), 
                                               ifelse(expo=="Age onset obesity", exp(estimate*sd((pop.analysis%>%filter(sexe=="D",meno_status_index==1))$ageOnsetObese, na.rm = T)), 
                                                      ifelse(expo=="Yearly BMI change", exp(estimate*sd((pop.analysis%>%filter(sexe=="D",meno_status_index==1))$trajectorySlope)),
                                                             ifelse(expo=="BMI at index", exp(estimate*sd((pop.analysis%>%filter(sexe=="D",meno_status_index==1))$bmi_atindex)),
                                                                    "Missing")))))))),
         low=ifelse(expo=="Duration overweight", exp(conf.low*sd((pop.analysis%>%filter(sexe=="D",meno_status_index==1))$yearsBMILimit)),
                    ifelse(expo=="Duration obesity", exp(conf.low*sd((pop.analysis%>%filter(sexe=="D",meno_status_index==1))$yearsBMI30more)),
                           ifelse(expo=="Cumulative overweight", exp(conf.low*sd((pop.analysis%>%filter(sexe=="D",meno_status_index==1))$cumOverLimit)), 
                                  ifelse(expo=="Cumulative obesity", exp(conf.low*sd((pop.analysis%>%filter(sexe=="D",meno_status_index==1))$cumObese)), 
                                         ifelse(expo=="Age onset overweight", exp(conf.low*sd((pop.analysis%>%filter(sexe=="D",meno_status_index==1))$ageOnsetOverLimit, na.rm = T)), 
         ...
         up=ifelse(expo=="Duration overweight", exp(conf.high*sd((pop.analysis%>%filter(sexe=="D",meno_status_index==1))$yearsBMILimit)),
                   ifelse(expo=="Duration obesity", exp(conf.high*sd((pop.analysis%>%filter(sexe=="D",meno_status_index==1))$yearsBMI30more)),
                          ifelse(expo=="Cumulative overweight", exp(conf.high*sd((pop.analysis%>%filter(sexe=="D",meno_status_index==1))$cumOverLimit)), 
                                 ifelse(expo=="Cumulative obesity", exp(conf.high*sd((pop.analysis%>%filter(sexe=="D",meno_status_index==1))$cumObese)), 
                                        ifelse(expo=="Age onset overweight", exp(conf.high*sd((pop.analysis%>%filter(sexe=="D",meno_status_index==1))$ageOnsetOverLimit, na.rm = T)), 
...
# paste0("Duration overweight", round(sd(pop.analysis$yearsBMILimit),2)),
# ifelse(expo=="Duration obesity", exp(conf.high*sd(pop.analysis$yearsBMI30more)),
#        ifelse(expo=="Cumulative overweight", exp(conf.high*sd(pop.analysis$cumOverLimit)), 
#               ifelse(expo=="Cumulative obesity", exp(conf.high*sd(pop.analysis$cumObese)), 
#                      ifelse(expo=="Age onset overweight", exp(conf.high*sd(pop.analysis$ageOnsetOverLimit, na.rm = T)), 
...
+
  scale_y_discrete(labels = rev(c("BMI at index date",
                                  expression("Duration BMI">="25<30"),
                                  
                                  expression("Cum. exposure BMI">="25<30"),
                                
                                  expression("Age onset BMI">="25<30")
                                )))+
  scale_color_manual(values=rev(c("black","#03992A",  "#32A5DD","#C51A95")))+
...
ggsave("exposureReviewer.png",
       dpi=300,
       width = 9, height = 9)

table <- toplotSD %>% select("cancer"  ,  "expo"   ,   "Type"  ,    "HR1"     ,   "low" ,     "up")
table <- table %>% mutate(HR1= paste0(format(round(HR1,2), nsmall=2), " (",
                                      format(round(low, 2), nsmall=2),"-",format(round(up,2), nsmall=2),
                                      ")")) %>% select(-low, -up, -Type)
table <- table %>% pivot_wider(names_from=expo, values_from=HR1)
table[match(rev(order.cancer1), table$cancer),] %>% htmlTable::htmlTable()
