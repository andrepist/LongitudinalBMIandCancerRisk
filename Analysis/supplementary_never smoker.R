...
#sensibility - restricted smoking -----


out <- list()
harrell<-list()
i = 0
current.pop<-pop.analysis%>%filter(smoking=="Never smoker") %>%  group_by(.imp)
...
saveRDS(df, "out.neversmokers.rds")
saveRDS(char, "charlson.rds")

#plot ------
df <- readRDS("out.neversmokers.rds")
pop.never <- pop.analysis %>% filter(smoking=="Never smoker")
