...
for (cancer in canceres){
  ...
  for (exposure in exposures){
    ...
    test.pool <- test %>%
      .[[-1]]%>% 
      pool()
    tidy <- tidy(test.pool, conf.int = TRUE, conf.level=level)
    i <- i+1
    out[[i]]<-tidy[1,] %>%
...
saveRDS(df, "out.testingCumObese.rds")

# plot ----

df<- readRDS("out.testingCumObese.rds")
...
htmlTable::htmlTable(toplotSD %>% select(HR1, cancer, expo, low, up, N))

ggsave("exposureMultipleTesting.png",
       dpi=300,
       width = 9, height = 9)
