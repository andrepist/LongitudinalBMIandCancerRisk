...
test2 <- test2 %>% mutate(overLimit=ifelse(value>=25&value<30,1, 0),
                         
                          ageOverLimit=ifelse(value>=25&value<30,as.numeric(substr(name, 5,6)), 999),
                         
                          deltaOverLimit=ifelse(value>24.9&value<30,value-24.9, 0)
                         )
test2 <- test2 %>% group_by(id, .imp) %>% mutate(yearsBMILimit=sum(overLimit),
                                               
                                                 ageOnsetOverLimit=min(ageOverLimit),
                                                 
                                                 cumOverLimit=sum(deltaOverLimit))

test2 <- test2 %>% mutate(everOverLimit=ifelse(yearsBMILimit==0, 0, 1))
test2 <- test2 %>% ungroup()
test2 <- test2 %>% mutate(ageOnsetOverLimit=ifelse(ageOnsetOverLimit==999, NA, ageOnsetOverLimit))
