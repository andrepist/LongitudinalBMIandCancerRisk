...
#sensibility - age <65 or more -----


out <- list()
harrell<-list()
i = 0
current.pop<-pop.analysis%>%filter(age.index<65) %>%  group_by(.imp)
...
toplotSD <- rbind(to65less %>% mutate(strata="<65"),
                  to65more %>% mutate(strata="\u226565"))
...
toplotSD<-toplotSD %>% 
  mutate(expostrata=paste0(expo, " (", strata, ")"),
         expostrata= factor(expostrata, levels=rev(c("BMI at index date (<65)","BMI at index date (\u226565)",
                                                     "Duration BMI \u226525 (<65)","Duration BMI \u226525 (\u226565)", 
                                                     "Duration BMI \u226530 (<65)", "Duration BMI \u226530 (\u226565)",
                                                     "Cum. exposure BMI \u226525 (<65)", "Cum. exposure BMI \u226525 (\u226565)",
                                                     "Cum. exposure BMI \u226530 (<65)", "Cum. exposure BMI \u226530 (\u226565)",
                                                     "Age of onset BMI \u226525 (<65)" ,"Age of onset BMI \u226525 (\u226565)" ,
                                                     "Age of onset BMI \u226530 (<65)", "Age of onset BMI \u226530 (\u226565)"
         ))) )

Nm <- toplotSD %>% filter(strata=="<65") %>% rename(Nm=N) %>% select(Nm,cancer)
Nf <- toplotSD %>% filter(strata=="\u226565") %>% rename(Nf=N)%>% select(Nf, cancer)

toplotSD <- toplotSD %>% left_join(Nm) %>% left_join(Nf) %>% distinct()

toplotSD <- toplotSD %>% 
  mutate(label=trimws(paste0(cancer, "\n(N <65 = ", trimws(format(Nm, big.mark=",")),"\nN \u226565 = " ,
...
Ncasos <- pop.analysis %>% filter(age.index>=65,everObese==1) %>% 
